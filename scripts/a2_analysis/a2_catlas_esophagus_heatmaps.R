#!/usr/bin/env Rscript
#
# A2: CATLAS Esophageal cCRE Enrichment Heatmaps for META Clusters
#
# Tests whether genes near each bNMF cluster's variants are enriched for
# chromatin accessibility (cCREs from sci-ATAC-seq) in esophageal cell types.
...
#
# Usage:
#   Rscript scripts/a2_analysis/a2_catlas_esophagus_heatmaps.R
...
out_dir <- file.path(base_dir, "results/a2_analysis/catlas/catlas_esophagus")
...
# Display labels for esophageal cell types (BED filename → nice label)
celltype_labels <- c(
...
message("=== A2: CATLAS Esophageal cCRE Enrichment Heatmaps ===\n")
...
if (length(bed_files) == 0) {
  stop("No BED files found in ", ccre_dir,
       ". Run download_catlas_esophagus.sh first.")
}
...
ccre_csv <- file.path(out_dir, "catlas_esophagus_ccre_gene_matrix.csv")
...
# Color scale: purple/violet gradient (same as CATLAS pancreas)
col_fun <- colorRamp2(
...
for (cl in names(cluster_labels)) {
  ...
  png_path <- file.path(out_dir, sprintf("catlas_esophagus_%s.png", cl))
  png(png_path, width = 6, height = panel_h, units = "in", res = 600)
  draw(hm_list,
       ht_gap = unit(4, "mm"),
       column_title = sprintf("Esophagus — %s", cluster_labels[cl]),
...
out_csv <- file.path(out_dir, "catlas_esophagus_permtest_results.csv")
...
if (length(imgs) > 0) {
  ...
  if (length(part1_cls) > 0) {
    place_panels(
      ...
      out_path = file.path(out_dir, "catlas_esophagus_combined_part1.png"),
  ...
  if (length(part2_cls) > 0) {
    place_panels(
      ...
      panel_w = panel_w, panel_h = panel_h,
      out_path = file.path(out_dir, "catlas_esophagus_combined_part2.png"),
      cluster_labels = cluster_labels,
      letter_offset = 6
    )
  }
