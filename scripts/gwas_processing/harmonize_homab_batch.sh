#!/bin/bash
#BSUB -J harmonize_homab
#BSUB -P acc_paul_oreilly
#BSUB -q premium
#BSUB -n 32
#BSUB -R "rusage[mem=64000]"
#BSUB -W 24:00
#BSUB -o logs/lsf/harmonize_homab_%J.out
#BSUB -e logs/lsf/harmonize_homab_%J.err

# Harmonize 2 HOMA-B GWAS files from 2 studies:
#   Dupuis NatGenet 2010 (1: EUR) — tab-sep, beta + odds_ratio column (all NA, handled by detect_params OR-drop logic), no N
#   Chebii Diabetologia 2025 (1: AFR) — tab-sep, beta, no N
# No reference GWAS — first HOMAB batch.
...
FILES=(
    # Dupuis NatGenet 2010 (tab-sep, beta + odds_ratio NA, no N)
    "Dupuis_NatGenet_2010.HOMAB.EUR.GRCh37.txt.gz|EUR"
    # Chebii Diabetologia 2025 (tab-sep, beta, no N)
    "Chebii_Diabetologia_2025.HOMAB.AFR.GRCh37.txt.gz|AFR"
)
...
"
    ...
    # Harmonize (no reference GWAS — first HOMAB batch)
    echo "  Harmonizing..."
...
echo ""
echo "=== HOMAB batch harmonization complete ==="
echo "  Total: ${TOTAL}, Processed: ${COUNT}, Failed: ${FAILED}"
echo "  Harmonized HOMAB files: $(ls ${HARMONIZED}/*HOMAB*.processed.txt.gz 2>/dev/null | wc -l)"
