#!/bin/bash
#BSUB -J harmonize_creatinine
#BSUB -P acc_paul_oreilly
#BSUB -q premium
#BSUB -n 32
#BSUB -R "rusage[mem=64000]"
#BSUB -W 12:00
#BSUB -o logs/lsf/harmonize_creatinine_%J.out
#BSUB -e logs/lsf/harmonize_creatinine_%J.err

# Harmonize 6 creatinine GWAS files: Karczewski (3), Karjalainen (1), Chen (1), Sakaue (1).
# All files are GRCh37 — no liftover needed.
# No reference GWAS available (first creatinine batch).
...
# No reference GWAS — first creatinine batch

# Files to harmonize: raw_filename|ancestry
FILES=(
    # Karczewski 2025 (BETA, GWAS Catalog format, no N column)
    "Karczewski_NatGenet_2025.Creatinine.META.GRCh37.txt.gz|META"
    "Karczewski_NatGenet_2025.Creatinine.EUR.GRCh37.txt.gz|EUR"
    "Karczewski_NatGenet_2025.Creatinine.SAS.GRCh37.txt.gz|SAS"
    # Karjalainen 2024 (GWAS Catalog format)
    "Karjalainen_Nature_2024.Creatinine.META.GRCh37.tsv.gz|META"
    # Chen 2023 (METAL format, Weight column)
    "Chen_CellGenomics_2023.Creatinine.EAS.GRCh37.tsv.gz|EAS"
    # Sakaue 2021 (SAIGE output format)
    "Sakaue_NatGenet_2021.Creatinine.EAS.GRCh37.txt.gz|EAS"
)
...
"
    ...
    # Build harmonize command (no reference GWAS for creatinine)
...
echo ""
echo "=== Creatinine batch harmonization complete ==="
echo "  Total: ${TOTAL}, Processed: ${COUNT}, Failed: ${FAILED}"
echo "  Harmonized creatinine files: $(ls ${HARMONIZED}/*Creatinine*.processed.txt.gz 2>/dev/null | wc -l)"
