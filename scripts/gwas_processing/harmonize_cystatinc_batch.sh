#!/bin/bash
#BSUB -J harmonize_cystatinc
#BSUB -P acc_paul_oreilly
#BSUB -q premium
#BSUB -n 32
#BSUB -R "rusage[mem=64000]"
#BSUB -W 12:00
#BSUB -o logs/lsf/harmonize_cystatinc_%J.out
#BSUB -e logs/lsf/harmonize_cystatinc_%J.err

# Harmonize 3 cystatin-C GWAS files: Karczewski (3).
# All files are GRCh37 — no liftover needed.
# No reference GWAS available (first cystatin-C batch).
...
# No reference GWAS — first cystatin-C batch

# Files to harmonize: raw_filename|ancestry
FILES=(
    # Karczewski 2025 (BETA, GWAS Catalog format, no N column)
    "Karczewski_NatGenet_2025.CystatinC.META.GRCh37.txt.gz|META"
    "Karczewski_NatGenet_2025.CystatinC.EUR.GRCh37.txt.gz|EUR"
    "Karczewski_NatGenet_2025.CystatinC.SAS.GRCh37.txt.gz|SAS"
)
...
"
    ...
    # Build harmonize command (no reference GWAS for cystatin-C)
...
echo ""
echo "=== Cystatin-C batch harmonization complete ==="
echo "  Total: ${TOTAL}, Processed: ${COUNT}, Failed: ${FAILED}"
echo "  Harmonized cystatin-C files: $(ls ${HARMONIZED}/*CystatinC*.processed.txt.gz 2>/dev/null | wc -l)"
