#!/bin/bash
#BSUB -J harmonize_bmr
#BSUB -P acc_paul_oreilly
#BSUB -q premium
#BSUB -n 32
#BSUB -R "rusage[mem=64000]"
#BSUB -W 12:00
#BSUB -o logs/lsf/harmonize_bmr_%J.out
#BSUB -e logs/lsf/harmonize_bmr_%J.err

# Harmonize 4 BMR (Base Metabolic Rate) GWAS files:
#   Karczewski/Pan-UKB 2025 (4: META, EUR, AFR, SAS) — GWAS Catalog format, no N column
...
FILES=(
    # Karczewski/Pan-UKB 2025 (GWAS Catalog format, no N)
    "Karczewski_NatGenet_2025.BMR.META.GRCh37.txt.gz|META"
    "Karczewski_NatGenet_2025.BMR.EUR.GRCh37.txt.gz|EUR"
    "Karczewski_NatGenet_2025.BMR.AFR.GRCh37.txt.gz|AFR"
    "Karczewski_NatGenet_2025.BMR.SAS.GRCh37.txt.gz|SAS"
)
...
"
    ...
    # Harmonize (no reference GWAS — first BMR batch, no pre-existing harmonized files)
    echo "  Harmonizing..."
...
echo ""
echo "=== BMR batch harmonization complete ==="
echo "  Total: ${TOTAL}, Processed: ${COUNT}, Failed: ${FAILED}"
echo "  Harmonized BMR files: $(ls ${HARMONIZED}/*BMR*.processed.txt.gz 2>/dev/null | wc -l)"
